...
def parse_translations_from_file_new(filename):
    translation = namedtuple("translation", "Id,Src,Translation,Bigram_Lprob,Trigram_Lprob,Penalty_Short,Penalty_Long,Edit_Distance,Average_TER,Bigram_mismatch_percentage,Trigram_mismatch_percentage,Worker_Bigram_Lprob,Worker_Trigram_Lprob,Worker_Penalty_Short,Worker_Penalty_Long,Worker_Edit_Distance,Worker_Average_TER,Worker_Bigram_mismatch_percentage,Worker_Trigram_mismatch_percentage,IsEnglishNative,IsUrduNative,LocationIndia,LocationPakistan,YearSpeakingEnglish,YearSpeakingUrdu")
    # translation = namedtuple("translation", "id, src, turk_translation, bigram_prob, trigram_prob, worker_id")
    ...
    for line in open(filename):
        ...
        tokens = ln_split[0:2];
        turk_translations = ln_split[2:6];
        fvs = [ -1*float(f) for f in ln_split[6:] ];
        sent_translations = [];

    	for (i,ind) in enumerate(range(0,22*4,22)) :
            fv = fvs[ind:ind+22];
	    sent_translations.append(translation(tokens[0],tokens[1],turk_translations[i],fv[0],fv[1],fv[2],fv[3],fv[4],fv[5],fv[6],fv[7],fv[8],fv[9],fv[10],fv[11],fv[12],fv[13],fv[14],fv[15],fv[16],fv[17],fv[18],fv[19],fv[20],fv[21]))	
        
	total_translations.extend(sent_translations)
    return total_translations
